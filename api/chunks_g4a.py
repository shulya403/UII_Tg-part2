...
class Chunk():
    ...
    #Подкачка эмбеддинга без Hugging face
    def My_External_Embed(self):
        print("начили")
        try:
           # Загружаем предобученную модель word2vec для русского языка
           #print(json.dumps(api.info(), indent=4))
           fasttext_model = api.load("word2vec-ruscorpora-300")
           print("грузанулось")

           class FastTextEmbeddings(Embeddings):
                    # Метод для получения эмбеддингов для списка документов
                    def embed_documents(self, texts: list) -> list:

                        embeddings_list = []
                        for text in texts:
                            tokens = text.split()  # Разбиваем текст на токены
                            token_embeddings = [fasttext_model[word] for word in tokens if word in fasttext_model]
                            if len(token_embeddings) > 0:
                                # Среднее эмбеддингов всех токенов
                                document_embedding = sum(token_embeddings) / len(token_embeddings)
                                embeddings_list.append(document_embedding)
                            else:
                                # Если ни одного токена не найдено, возвращаем нулевой вектор
                                embeddings_list.append(np.zeros(fasttext_model.vector_size))
                        return embeddings_list

                    # "Метод для получения эмбеддингов для запроса"
                    def embed_query(self, text: str) -> np.ndarray:

                        tokens = text.split()  # Разбиваем текст на токены
                        token_embeddings = [fasttext_model[word] for word in tokens if word in fasttext_model]
                        if len(token_embeddings) > 0:
                            query_embedding = sum(token_embeddings) / len(token_embeddings)
                        else:
           ...
           return embeddings
                ...
                print(f"Ошибка при загрузке FastText Russian Model: {e}")
                return None
    ...
    def base_load(self):
        ...
        embeddings = self.My_External_Embed()
            ...
            async with session.post(
                ...
                try:
                    ...
                    rate_limit_headers = {
                        ...
                        'reset_requests': int(
                            float(response.headers.get('x-ratelimit-reset-requests').replace('s', '')) * 1000)
                    }
